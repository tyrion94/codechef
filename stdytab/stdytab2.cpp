...
int ncr(int n,int r)
{
 ...
 else
 {
  ...
  {
   memory[n][r]=(ncr(n-1,r)%c+ncr(n-1,r-1)%c)%c;
   return memory[n][r];
  }
 }
}
long long int memory2[2010][2010];
long long int no_of_martrix_with_sum(int n,int m,int sum)
{
 ...
 {
   memory2[n][sum]=(ncr(m-1+sum,m-1)*(no_of_martrix_with_sum(n-1,m,sum))%c)%c;
   for(int i=sum-1;i>=0;i--)
   {
    memory2[n][sum]=(memory2[n][sum]+(((ncr(m-1+sum,m-1))*(no_of_martrix_with_sum(n-1,m,i))%c)%c)+c)%c;
   }
   return memory2[n][sum];
 }
}
